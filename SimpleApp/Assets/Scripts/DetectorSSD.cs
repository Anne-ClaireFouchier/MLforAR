...
public class DetectorSSD : MonoBehaviour//, Detector
{

    public NNModel modelFile;
    public TextAsset labelsFile; // labelAsset
    ...
    public string OUTPUT_NAME = "Softmax";

    //This has to stay a const
    private const int _image_size = 224;
    ...
    public void Start() {
		...
		// create a model with the model file
        // later use this.worker = GraphicsWorker.GetWorker(model);
        worker = WorkerFactory.CreateWorker(WorkerFactory.Type.ComputePrecompiled, model);
		// load the labels in the memory
        LoadLabels();
	}

	void LoadLabels() {
		//get only items in quotes : split by quaotation
		var stringArray = labelsFile.text.Split('"').Where((item, index) => index % 2 != 0);
		//get every other item
		this.labels = stringArray.Where((x, i) => i % 2 != 0).ToArray();
	}
